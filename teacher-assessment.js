(function () {
  ...
  const schemas = {
    lessonPlans: {
      title: "Lesson Plan",
      fields: [
        ["period", "Month / Week", "text"], ["className", "Class", "class"],
        ["subject", "Subject", "text"], ["plannedTopic", "Chapter / Topic Planned", "textarea"],
    ...
    lessonTracking: {
      ...
      fields: [
        ["period", "Month / Week", "text"], ["className", "Class", "class"],
        ["subject", "Subject", "text"], ["plannedTopic", "Planned Lesson / Topic", "textarea"],
  ...
  function records(type, teacherId, useMonth) {
    return (data()[type] || []).filter((record) =>
      ...
      && (!useMonth || selectedMonth() === "All Months" || record.month === selectedMonth()
        || String(record.period || "").toLowerCase().includes(selectedMonth().toLowerCase())));
  }
  ...
  function lessonPlanMonthlyData(teacherId) {
    ...
    return months.map((month) => {
      const items = plans.filter((item) => String(item.period || "").toLowerCase().includes(month.toLowerCase()));
  ...
  function fieldControl(field, record) {
    ...
    if (type === "textarea") return `<label>${escapeHtml(label)}<textarea name="${key}" required>${escapeHtml(value)}</textarea></label>`;
    const choices = type === "class" ? app.classNames : type === "month" ? months : options;
    ...
    const required = type === "computed" ? " readonly" : " required";
